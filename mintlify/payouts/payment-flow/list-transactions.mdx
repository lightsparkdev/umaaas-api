---
title: "List Rewards Transactions"
...
Keep paginating while `hasMore` is `true`. Continue making requests with the `nextCursor` value from each response until `hasMore` becomes `false`.
...
For data that doesn't change frequently (like completed transactions), implement caching in your application. Completed transactions are immutable, so they're safe to cache using a key like `{customerId}-{month}` to reduce API calls and improve performance.

</Accordion>

<Accordion title="Use specific filters to reduce result sets">
Apply filters in the API query to get only the data you need, rather than fetching all transactions and filtering in your application code.

**Good approach:**
```bash
curl -X GET 'https://api.lightspark.com/grid/2025-10-13/transactions?status=FAILED&startDate=2025-10-01T00:00:00Z&type=OUTGOING' \
  -H 'Authorization: Basic <base64-encoded-credentials>'
```

**Bad approach:** Fetching all transactions without filters and filtering in your application code. This wastes bandwidth and increases latency.

</Accordion>

<Accordion title="Handle rate limits gracefully">
Implement exponential backoff for rate limit errors in your application. When you receive a `429 Too Many Requests` response:

1. Check the `Retry-After` header for the recommended wait time
2. Wait for the specified duration (in seconds)
3. Retry the request
4. Implement a maximum retry limit (e.g., 3 attempts) to prevent infinite loops

This ensures your application handles API rate limits gracefully without overwhelming the service.

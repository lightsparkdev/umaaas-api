...
To update your platform configuration for pay-to-bank payments, use the PATCH endpoint:
...
The `supportedCurrencies` array allows you to define settings for each currency your platform will support for pay-to-bank payments. Each object in this array can contain:
...
<Note>
For pay-to-bank payments, you do not need UMA-specific fields like `umaProviderRequiredCustomerFields` or `umaProviderRequiredCounterpartyCustomerFields`. The configuration is simpler and focuses on basic currency requirements.
</Note>

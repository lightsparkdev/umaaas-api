...
These fields must be supplied when creating or updating users or counterparties via the respective API endpoints if they are expected to use the specified currency. Refer to the [Configuring Users](/docs/configuring-users) guide for more details on how this impacts user setup.

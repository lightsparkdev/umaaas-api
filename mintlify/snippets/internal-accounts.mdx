...
Check the `balance.amount` field in the response to verify sufficient funds before initiating transfers.
...
Parse the `fundingPaymentInstructions` array from the API response and select the appropriate instructions based on your customer's preferred payment method.
...
Internal accounts are ideal for scenarios where customers need to deposit funds in advance. For example, after a customer deposits $500 to their USD internal account, you can initiate a cross-currency transfer to Mexico:

```bash
# Create a quote for USD to MXN transfer
...
# Execute the transfer using the quote ID from the response
...
Use internal accounts to hold customer funds that will be used for multiple transactions. Check available balance across all currencies:
...
The response will include all internal accounts with their current balances in each currency.
...
Set up monitoring to alert customers when their balance is low. Implement low-balance checks in your application and send notifications with the account's `fundingPaymentInstructions` so customers know how to add funds.
</Accordion>

<Accordion title="Handle multiple currencies efficiently">
If your platform supports multiple currencies, organize internal accounts by currency in your UI. You can use the `currency` query parameter to filter for specific currencies when retrieving accounts, making lookups more efficient.
</Accordion>

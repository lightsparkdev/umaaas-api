...
export function useFormData() {
  ...
  useEffect(() => {
    const generateInitialData = (): FormData => {
      const domain = import.meta.env.UAAS_UMA_DOMAIN || window.location.hostname;
      return {
  ...
  const generateNewData = () => {
    const domain = import.meta.env.UAAS_UMA_DOMAIN || (typeof window !== 'undefined' ? window.location.hostname : 'localhost:3000');
    setFormData({
      platformUserId: faker.string.uuid(),
      umaAddress: `$${faker.internet.username().toLowerCase()}@${domain}`,
      userType: 'INDIVIDUAL',

...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw UmaaasInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw UmaaasInvalidDataException("`$name` is not set")
            is JsonNull -> throw UmaaasInvalidDataException("`$name` is null")
            else -> throw UmaaasInvalidDataException("`$name` is invalid, received $this")
        }
...
internal fun <T : Any> T?.getOrThrow(name: String): T =
    this ?: throw UmaaasInvalidDataException("`${name}` is not present")

...
import { Umaaas, ClientOptions } from 'umaaas';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Umaaas)`
    : `const run: (${functionSource.client}: Umaaas) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.users.update',
    'client.users.bulk.getJobStatus',
    'client.users.bulk.upload',

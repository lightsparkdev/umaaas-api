...
class PlatformCurrencyConfig(BaseModel):
    ...
    uma_provider_required_user_fields: Optional[List[UserInfoFieldName]] = Field(default=None, description="List of user PII field names that are required by the underlying UMA provider when creating a user for this currency. These fields must be supplied when creating or updating a user if this currency is intended to be used by that user. If no fields are required, this field is omitted.", alias="umaProviderRequiredUserFields")
